...
function Section({ title, children }: PropsWithChildren<{ title: string }>) {
  return (
    <View className="rounded-2xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900">
      <Text className="mb-3 text-lg font-bold text-slate-900 dark:text-white">{title}</Text>
      <Text className="text-[15px] leading-6 text-slate-600 dark:text-slate-300">{children}</Text>
    </View>
  );
}
...
export function TermsScreen() {
  ...
  return (
    <Screen scroll>
      <View className="mb-6 mt-4 items-center gap-2 rounded-2xl bg-slate-950 px-6 py-8">
      ...
      <View className="gap-4 pb-8">
        <View className="rounded-2xl border-2 border-red-200 bg-red-50 p-6 dark:border-red-900 dark:bg-red-950/30">
          <Text className="mb-2 text-lg font-bold text-red-800 dark:text-red-300">
            {t("legal.terms.simulationTitle")}
          </Text>
          <Text className="text-[15px] leading-6 text-red-700 dark:text-red-300">
