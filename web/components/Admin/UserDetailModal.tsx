...
export default function UserDetailModal() {
  ...
  const [phoneForm, setPhoneForm] = useState({ open: false, local: "", invalid: false });
  ...
  useEffect(() => {
    if (targetUser) {
      ...
      setPhoneForm({ open: false, local: targetUser.phone ? targetUser.phone.replace("+20", "") : "", invalid: false });
    }
  }, [targetUser]);
  ...
  function handleSavePhone(e: React.FormEvent) {
    ...
    if (!isValidEgyptianMobile(phoneForm.local)) {
      setPhoneForm((f) => ({ ...f, invalid: true }));
      return;
    }
    overridePhone.mutate(
      { userId: targetUserId, phone: toE164EgyptianPhone(phoneForm.local) },
      { onSuccess: () => setPhoneForm((f) => ({ ...f, open: false, invalid: false })) }
    );
  }
        ...
        <div className="p-5 flex flex-col gap-5">
          ...
          <form onSubmit={handleSaveProfile} className="flex flex-col gap-3">
            <div className="grid grid-cols-2 gap-3">
              <EditInput label="First name" value={form.firstName} onChange={(v) => setForm((f) => ({ ...f, firstName: v }))} disabled={locked} />
              <EditInput label="Last name" value={form.lastName} onChange={(v) => setForm((f) => ({ ...f, lastName: v }))} disabled={locked} />
            </div>
            <EditInput label="Address" value={form.address} onChange={(v) => setForm((f) => ({ ...f, address: v }))} disabled={locked} />
            ...
            {phoneForm.open && (
              <form onSubmit={handleSavePhone} className="flex flex-col gap-2 mt-2">
                <PhoneField
                  ...
                  onChange={(v) => setPhoneForm((f) => ({ ...f, local: v, invalid: false }))}
